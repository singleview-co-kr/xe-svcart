...
function addItemToCart(item_srl) {
	...
	exec_xml('svcart', 'procSvorderAddItemsToCart', params, function(ret_obj) {
		load_cart();
		open_cart('cart');
		//location.href = current_url;
...
function addItemsToFavorites(item_srl) {
	...
	exec_xml('svcart', 'procSvorderAddItemsToFavorites', params, function(ret_obj) {
		load_favorites();
		open_cart('favorites');
/*
...
(function($) {
	jQuery(function($) {
		...
		$('.updateQuantity').live('click', function() {
			...
			var responses = ['error','message'];
			exec_xml('svcart', 'procSvcartUpdateQuantity', params, completeDeleteFavoriteItems, responses);
		});
	});
}) (jQuery);
